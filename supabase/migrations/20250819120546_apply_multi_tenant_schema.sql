...
-- Applying fresh to ensure remote database has the base schema
...
-- Use IF NOT EXISTS equivalent for columns
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name='businesses' AND column_name='stakeholder_id'
  ) THEN
...
-- Insert default plan features (only if they don't exist)
INSERT INTO plan_features (plan, max_shops, ad_tier) 
SELECT * FROM (VALUES
  ('basic', 1, 'none'),
  ('pro', 5, 'standard'),
  ('enterprise', 50, 'top')
) AS v(plan, max_shops, ad_tier)
WHERE NOT EXISTS (SELECT 1 FROM plan_features WHERE plan = v.plan);
...
DROP POLICY IF EXISTS sel_vouchers ON vouchers;
...
DROP POLICY IF EXISTS ins_vouchers ON vouchers;
CREATE POLICY ins_vouchers ON vouchers
FOR INSERT WITH CHECK (
  is_platform_admin(auth.uid())
  OR created_by = auth.uid() -- Allow users to create vouchers they create
);

-- RLS Policies for advertisements
DROP POLICY IF EXISTS sel_advertisements ON advertisements;
...
DROP POLICY IF EXISTS ins_advertisements ON advertisements;
CREATE POLICY ins_advertisements ON advertisements
FOR INSERT WITH CHECK (
  is_platform_admin(auth.uid())
  OR created_by = auth.uid() -- Allow users to create advertisements they create
);

-- Create indexes for performance
CREATE INDEX IF NOT EXISTS idx_stakeholders_status ON stakeholders(status);
CREATE INDEX IF NOT EXISTS idx_memberships_user_id ON memberships(user_id);
CREATE INDEX IF NOT EXISTS idx_memberships_stakeholder_id ON memberships(stakeholder_id);
CREATE INDEX IF NOT EXISTS idx_memberships_shop_id ON memberships(shop_id);
CREATE INDEX IF NOT EXISTS idx_subscriptions_stakeholder_id ON subscriptions(stakeholder_id);
...
CREATE INDEX IF NOT EXISTS idx_advertisements_shop_id ON advertisements(shop_id);
CREATE INDEX IF NOT EXISTS idx_advertisements_dates ON advertisements(start_at, end_at);
CREATE INDEX IF NOT EXISTS idx_businesses_stakeholder_id ON businesses(stakeholder_id);
